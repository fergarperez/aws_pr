...
AWS_EC2_INSTANCE_ID=$(aws ec2 run-instances \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=EC2FERNANDO_1}]' \
...
AWS_Subred_CIDR_BLOCK_PRIVADA=10.22.2.0/24
AWS_IP_UbuntuServer_PRIVADA=10.22.2.100
...
AWS_ID_SubredPrivada=$(aws ec2 create-subnet \
  --vpc-id $AWS_ID_VPC \
  --cidr-block $AWS_Subred_CIDR_BLOCK_PRIVADA \
  --availability-zone us-east-1b \
  --query 'Subnet.{SubnetId:SubnetId}' \
  --output text)

###############################################################################
#################      NAT GATEWAY Y TABLA DE ENRUTAMIENTO      ###############
###############################################################################

echo ""
echo "######################################################################"
echo "Creando NAT Gateway y Tabla de Enrutamiento..."
AWS_ID_TablaEnrutamiento=$(aws ec2 create-route-table \
  --vpc-id $AWS_ID_VPC \
  --query 'RouteTable.{RouteTableId:RouteTableId}' \
  --output text)

## Crear una ruta para enviar tráfico de la subred privada al NAT Gateway
aws ec2 create-route \
  --route-table-id $AWS_ID_TablaEnrutamiento \
  --destination-cidr-block 0.0.0.0/0 \
  --nat-gateway-id <YOUR_NAT_GATEWAY_ID>

## Asociar la tabla de enrutamiento con la subred privada
aws ec2 associate-route-table \
  --subnet-id $AWS_ID_SubredPrivada \
  --route-table-id $AWS_ID_TablaEnrutamiento

## Crear una Elastic IP para el NAT Gateway
AWS_ElasticIP_NAT=$(aws ec2 allocate-address --domain vpc --query 'AllocationId' --output text)

## Crear el NAT Gateway
AWS_ID_NATGateway=$(aws ec2 create-nat-gateway \
  --subnet-id $AWS_ID_SubredPublica \
  --allocation-id $AWS_ElasticIP_NAT \
  --query 'NatGateway.{NatGatewayId:NatGatewayId}' \
  --output text)

###############################################################################
####################       UBUNTU SERVER PRIVADO     #########################
###############################################################################

## Crear un grupo de seguridad Ubuntu Server Privado
echo ""
echo "######################### Ubuntu Server Privado #######################"
echo "######################################################################"
echo "Creando grupo de seguridad Ubuntu Server Privado..."
AWS_ID_GrupoSeguridad_Ubuntu_PRIVADO=$(aws ec2 create-security-group \
  --vpc-id $AWS_ID_VPC \
  --group-name $AWS_Proyecto-us-sg-privado \
  --description "$AWS_Proyecto-us-sg-privado" \
  --output text)

echo "ID Grupo de seguridad de ubuntu_privado: " $AWS_ID_GrupoSeguridad_Ubuntu_PRIVADO

echo "Añadiendo reglas de seguridad al grupo de seguridad Ubuntu Server Privado..."
## Abrir los puertos de acceso a la instancia privada
aws ec2 authorize-security-group-ingress \
  --group-id $AWS_ID_GrupoSeguridad_Ubuntu_PRIVADO \
  --ip-permissions '[{"IpProtocol": "tcp", "FromPort": 22, "ToPort": 22, "IpRanges": [{"CidrIp": "'$AWS_Subred_CIDR_BLOCK_PRIVADA'", "Description": "Allow SSH"}]}]'

## Añadirle etiqueta al grupo de seguridad privado
echo "Añadiendo etiqueta al grupo de seguridad Ubuntu Server Privado..."
aws ec2 create-tags \
--resources $AWS_ID_GrupoSeguridad_Ubuntu_PRIVADO \
--tags "Key=Name,Value=$AWS_Proyecto-us-sg-privado"

###############################################################################
## Crear una instancia EC2 Privada (con una imagen de ubuntu 22.04)
###############################################################################
echo ""
echo "Creando instancia EC2 Ubuntu Privada ##################################"
AWS_EC2_INSTANCE_ID_PRIVADO=$(aws ec2 run-instances \
  --image-id $AWS_AMI_Ubuntu_ID \
  --instance-type t2.micro \
  --key-name vockey \
  --monitoring "Enabled=false" \
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=EC2FERNANDO_PRIVADO}]' \
  --security-group-ids $AWS_ID_GrupoSeguridad_Ubuntu_PRIVADO \
  --subnet-id $AWS_ID_SubredPrivada \
  --private-ip-address $AWS_IP_UbuntuServer_PRIVADA \
  --query 'Instances[0].InstanceId' \
  --output text)

echo "Instancia EC2 Privada creada con ID: $AWS_EC2_INSTANCE_ID_PRIVADO"
